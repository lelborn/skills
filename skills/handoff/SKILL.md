...
- **Reference, don't restate.** If a spec, PR, issue, commit, or doc exists, point to it by path/URL and say what it is; don't paste it (that just creates a second copy that drifts). The thing worth capturing that *isn't* already written down is the reasoning behind decisions.
