...
Pick the type from what the change *does*, not what the user *called* it. "Clean up the login page" could be a `fix`, `refactor`, or `style`; look at `git diff --staged` before deciding.
...
- **Body** — optional, one blank line after the description. Explain the *why*; don't narrate the diff line by line. Include only when the reason isn't obvious.
