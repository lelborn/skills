...
2. **Classify each change by its effect on that surface**, not by the size of the diff: does it *break* existing use, *add* compatible capability, or *fix* behaviour inside the existing contract? A wholly new, opt-in unit — a new skill, a new subcommand — is additive, not breaking: nobody's existing use changes.
...
**Under `0.y.z`** — initial development, `1.0.0` uncut, so there's no promise; the numbers signal *relative* risk instead. The scale slides down one slot, matching how `^` ranges resolve (`^0.1.0` means `>=0.1.0 <0.2.0`, so MINOR is the breaking position):

1. Backward-incompatible change → **MINOR** (`0.3.2` → `0.4.0`, reset PATCH to 0).
2. Anything compatible — feature *or* fix → **PATCH** (`0.3.2` → `0.3.3`).
